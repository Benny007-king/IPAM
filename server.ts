...
// The app is typically served over plain HTTP behind an IIS reverse proxy,
// so cookies must NOT be flagged `secure` (that would stop the browser from
// ever storing/sending the auth cookie, breaking login). Set COOKIE_SECURE=true
// only when serving over HTTPS. `sameSite: 'lax'` works for same-origin logins.
const COOKIE_SECURE = process.env.COOKIE_SECURE === 'true';
const COOKIE_OPTIONS = {
  httpOnly: true,
  secure: COOKIE_SECURE,
  sameSite: (COOKIE_SECURE ? 'none' : 'lax') as 'none' | 'lax',
  maxAge: 24 * 60 * 60 * 1000, // 1 day, matches JWT expiry
};

// Escape values interpolated into LDAP search filters (RFC 4515) to prevent
// LDAP filter injection via the login username.
...
async function startServer() {
        ...
        for (const server of ldapServers) {
          ...
          for (const address of addresses) {
            ...
            try {
              ...
              const sAMAccountName = escapeLdapFilter(username.split('@')[0].split('\\').pop());
  ...
  app.post('/api/auth/logout', (req, res) => {
    res.clearCookie('token', { httpOnly: true, secure: COOKIE_SECURE, sameSite: COOKIE_OPTIONS.sameSite });
